...
namespace ChocDino.HQText.Editor
{
	...
	internal class AnimCollapseSection {
		internal const string SettingsPrefix = "HQText-CoreComponentEditor";
		...
		public AnimCollapseSection(string label,
								bool showOnlyInEditMode,
								bool isDefaultExpanded,
								System.Action action,
								UnityEditor.Editor editor,
								Color backgroundColor,
								List<AnimCollapseSection> groupItems = null)
		...
		public AnimCollapseSection(GUIContent label,
								bool showOnlyInEditMode,
								bool isDefaultExpanded,
								System.Action action,
								UnityEditor.Editor editor,
		...
		~AnimCollapseSection() { _anim.valueChanged.RemoveAllListeners(); }
		...
		get { return _anim.target; }
		set {
			if (_anim.target != value) {
			_anim.target = value;
			if (value)
				CollapseSiblings();
			}
		...
		get { return GetPrefName(_name); }
		}

		public void Save() { EditorPrefs.SetBool(PrefName, IsExpanded); }
		...
		internal static string GetPrefName(string label) { return SettingsPrefix + "Expand-" + label; }
		...
		if (EditorGUIUtility.isProSkin) {
			GUI.backgroundColor =
				section.BackgroundColor * new Color(headerGlow, headerGlow, headerGlow, 1f);
		} else {
		...
		internal static void Show(string label,
								ref bool isExpanded,
								System.Action action,
								bool showOnlyInEditMode) {
		if (showOnlyInEditMode && Application.isPlaying)
			return;
		...
		internal static void EndShow() { GUILayout.EndVertical(); }
	}
}