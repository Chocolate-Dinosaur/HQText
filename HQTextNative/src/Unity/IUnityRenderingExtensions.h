...
/*
    ...
    On top of the Low-level native plugin interface, Unity also supports low
   level rendering extensions that can receive callbacks when certain events
   happen. This is mostly used to implement and control low-level rendering in
   your plugin and enable it to work with Unity’s multithreaded rendering.

    Due to the low-level nature of this extension the plugin might need to be
   preloaded before the devices get created. Currently the convention is
   name-based namely the plugin name must be prefixed by “GfxPlugin”. Example:
...
//     These events will be propagated to all plugins that implement void
//     UnityRenderingExtEvent(UnityRenderingExtEventType event, void* data);

enum UnityRenderingExtEventType {
  kUnityRenderingExtEventSetStereoTarget,  // issued during SetStereoTarget and
                                           // carrying the current 'eye' index
                                           // as parameter
  kUnityRenderingExtEventSetStereoEye,  // issued during stereo rendering at the
                                        // beginning of each eye's rendering
                                        // loop. It carries the current 'eye'
                                        // index as parameter
  kUnityRenderingExtEventStereoRenderingDone,  // issued after the rendering has
                                               // finished
  kUnityRenderingExtEventBeforeDrawCall,  // issued during BeforeDrawCall and
                                          // carrying
                                          // UnityRenderingExtBeforeDrawCallParams
                                          // as parameter
  kUnityRenderingExtEventAfterDrawCall,  // issued during AfterDrawCall. This
                                         // event doesn't carry any parameters
  kUnityRenderingExtEventCustomGrab,     // issued during GrabIntoRenderTexture
                                         // since we can't simply copy the
                                         // resources
  //      when custom rendering is used - we
  //      need to let plugin handle this. It
  //      carries over a
  //      UnityRenderingExtCustomBlitParams
  //      params = { X, source, dest, 0, 0 }
  //      ( X means it's irrelevant )
  kUnityRenderingExtEventCustomBlit,  // issued by plugin to insert custom
                                      // blits. It carries over
                                      // UnityRenderingExtCustomBlitParams as
                                      // param.
  ...
  kUnityRenderingExtEventUpdateTextureBeginV1 =
      kUnityRenderingExtEventUpdateTextureBegin,  // Deprecated. Issued to
                                                  // update a texture. It
                                                  // carries over
                                                  // UnityRenderingExtTextureUpdateParamsV1
  kUnityRenderingExtEventUpdateTextureEndV1 =
      kUnityRenderingExtEventUpdateTextureEnd,  // Deprecated. Issued to signal
                                                // the plugin that the texture
                                                // update has finished. It
                                                // carries over the same
                                                // UnityRenderingExtTextureUpdateParamsV1
                                                // as
                                                // kUnityRenderingExtEventUpdateTextureBeginV1
  kUnityRenderingExtEventUpdateTextureBeginV2,  // Issued to update a texture.
                                                // It carries over
                                                // UnityRenderingExtTextureUpdateParamsV2
  kUnityRenderingExtEventUpdateTextureEndV2,  // Issued to signal the plugin
                                              // that the texture update has
                                              // finished. It carries over the
                                              // same
                                              // UnityRenderingExtTextureUpdateParamsV2
                                              // as
...
enum UnityRenderingExtCustomBlitCommands {
  kUnityRenderingExtCustomBlitVRFlush,  // This event is mostly used in multi
                                        // GPU configurations ( SLI, etc ) in
                                        // order to allow the plugin to flush
                                        // all GPU's targets
...
enum UnityRenderingExtQueryType {
  kUnityRenderingExtQueryOverrideViewport =
      1 << 0,  // The plugin handles setting up the viewport rects. Unity will
               // skip its internal SetViewport calls
  kUnityRenderingExtQueryOverrideScissor =
      1 << 1,  // The plugin handles setting up the scissor rects. Unity will
               // skip its internal SetScissor calls
  kUnityRenderingExtQueryOverrideVROcclussionMesh =
      1 << 2,  // The plugin handles its own VR occlusion mesh rendering. Unity
               // will skip rendering its internal VR occlusion mask
  kUnityRenderingExtQueryOverrideVRSinglePass =
      1 << 3,  // The plugin uses its own single pass stereo technique. Unity
               // will only traverse and render the render node graph once.
               //      and it will clear the whole render target not just
               //      per-eye on demand.
  kUnityRenderingExtQueryKeepOriginalDoubleWideWidth_DEPRECATED =
      1 << 4,  // Instructs unity to keep the original double wide width. By
               // default unity will try and have a power-of-two width for
               // mip-mapping requirements.
  kUnityRenderingExtQueryRequestVRFlushCallback =
      1 << 5,  // Instructs unity to provide callbacks when the VR eye textures
...
struct UnityRenderingExtBeforeDrawCallParams {
  ...
  int eyeIndex;  // the index of the current stereo "eye" being currently
                 // rendered.
};
...
struct UnityRenderingExtCustomBlitParams {
  ...
  unsigned int
      command;  // command for the custom blit - could be any
                // UnityRenderingExtCustomBlitCommands command or custom ones.
  unsigned int commandParam;  // custom parameters for the command
  unsigned int commandFlags;  // custom flags for the command
};

// Deprecated. Use UnityRenderingExtTextureUpdateParamsV2 and
// CommandBuffer.IssuePluginCustomTextureUpdateV2 instead. Only supports DX11,
// GLES, Metal
struct UnityRenderingExtTextureUpdateParamsV1 {
  void*
      texData;  // source data for the texture update. Must be set by the plugin
...
// CommandBuffer.IssuePluginCustomTextureUpdateV2 instead. Only supports DX11,
// GLES, Metal
typedef UnityRenderingExtTextureUpdateParamsV1
    UnityRenderingExtTextureUpdateParams;

// Type of the "data" parameter passed when callbacks registered with
// CommandBuffer.IssuePluginCustomTextureUpdateV2 are called. Supports DX11,
// GLES, Metal, and Switch (also possibly PS4 in the future)
...
// Certain Unity APIs (GL.IssuePluginEventAndData,
// CommandBuffer.IssuePluginEventAndData) can callback into native plugins.
...
// If exported by a plugin, this function will be called for all the events in
// UnityRenderingExtEventType
void UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API
UnityRenderingExtEvent(UnityRenderingExtEventType event, void* data);
// If exported by a plugin, this function will be called to query the plugin for
// the queries in UnityRenderingExtQueryType
